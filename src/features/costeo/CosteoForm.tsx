...
export function CosteoForm({ value, onChange }: Props) {
  ...
  const addMp = () => set({ materiaPrima: [...value.materiaPrima, { id: rowId("mp"), nombre: "", medida: "", precioUnitario: 0, requerimiento: 0 }] });
    ...
    <Accordion type="multiple" defaultValue={["mp", "mod", "cf", "margen"]} className="space-y-3">
      ...
      <AccordionItem value="mp">
        ...
        <AccordionContent>
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs font-medium text-muted-foreground">
                <th className="pb-2 pl-1">Insumo</th><th className="pb-2 w-16">Medida</th>
                <th className="pb-2 w-24">Precio unit.</th><th className="pb-2 w-24">Requerim.</th>
                <th className="pb-2 w-20">Total</th><th className="w-9" />
              </tr>
            </thead>
            <tbody>
              {value.materiaPrima.map((x) => (
                <tr key={x.id}>
                  ...
                  <td className="py-1 pr-2"><Input value={x.medida} placeholder="kg" onChange={(e) => updMp(x.id, { medida: e.target.value })} /></td>
            ...
            <tfoot>
              <tr>
                <td colSpan={4} className="rounded-l-md bg-slate-50 py-2 pl-3 text-xs font-medium text-slate-700"><span className="inline-flex items-center gap-1"><Lock className="size-3 text-muted-foreground" /> Materia prima unitaria</span></td>
          ...
          <div className="mt-2"><Button variant="outline" size="sm" onClick={addMp}><Plus /> Agregar insumo</Button></div>
        </AccordionContent>
      </AccordionItem>
